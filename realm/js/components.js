const customComponents = {
	"bulk-button": {
		...
		template: template(/*html*/`<button :class="'upg tooltipBox ' + (player[layer].bulk.eq(bulkAmount) ? 'locked' : 'can')" :style="[
			(player[layer].bulk.eq(bulkAmount) ? {} : {'background-color': tmp[layer].color}),
			{
				width: 'min-content',
				'min-height': '30px',
				'border-radius': '5px',
	...
	"max-spell-cast": {
		...
		template: template(/*html*/`<button :class="'upg tooltipBox ' + (tmp[layer].clickables[data].canClick ? 'can' : 'locked')" :style="[
			(tmp[layer].clickables[data].canClick ? {'background-color': tmp[layer].clickables[data].color} : {}),
			{
				'border-radius': 0,
				width: '125px',
				'min-height': '25px',
	...
	"autocast-toggle": {
		...
		template: template(/*html*/`<div>
			<button v-for="mode in unlockedModes" :class="'upg tooltipBox ' + (canClick ? 'can' : 'locked')" :style="[
				(canClick ?
					{'background-color': (mode === 2 ? tmp[layer].color : tmp[layer].clickables[data].color)}
					: {}),
				{
				...
				<svg width="40" height="40" viewBox="0 0 40 40" fill="none" stroke="#000000" stroke-width="2">
					...
					<text v-if="mode === activeMode" x="20" y="19.9" text-anchor="middle" dominant-baseline="central" fill="#000000" stroke="none" style="font-size: 15px">
						{{ hasUpgrade(layer, 102) ? spellOrder + 1 : "ON" }}
					</text>
				</svg>
			</button>
		</div>`),
