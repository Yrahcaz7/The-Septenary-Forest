const manaUpgrades = [
	...
	["Mana Vessels", "multiply max mana based on your components", () => player.C.points.add(1).pow(0.125), "", "x"],
	["Magical Environment", "increase mana regen based on your components", () => player.C.points.add(1).pow(0.225), "+"],
...
const factionUpgrades = [[
	["Magic Dust", "multiply the first effect of basic components based on your mana regen", () => player.M.manaRegen.mul(4).add(1).pow(0.4), "", "x"],
	["Fairy Workers", "multiply the first effect of basic components based on your components", () => player.C.points.add(1).pow(0.2), "", "x"],
	["Fairy Traders", "multiply coins/click and faction coin find chance based on your components", () => player.C.points.add(1).pow(0.1), "", "x"],
	["Active Workers", "multiply coins/click based on your coins/sec", () => new Decimal(tmp.pointGen).add(1).log10().div(2).add(1), "", "x"],
	["Epitome of Mischief", "multiply the first effect of basic components based on your fairy coins", () => player.FC[0].add(1).pow(0.3), "", "x"],
	["The Roots of Mana", "multiply mana regen based on your basic components", () => [11, 12, 13].reduce((acc, id) => acc.add(getBuyableAmount("C", id)), newDecimalOne()).pow(0.05), "", "x"],
], [
	["Super Clicks", "multiply coins/click based on your components", () => player.C.points.add(1).pow(0.3), "", "x"],
	...
	["Demonic Prestige", "multiply coins/sec based on your component tiers", () => player.C.tiers.div(5).add(1), "", "x"],
	["Demonic Blood", "multiply blood frenzy effect based on your components (higher numbered ones count more)", () => [11, 12, 13, 14].reduce((acc, id, num) => acc.add(getBuyableAmount("C", id).mul(5 ** num)), newDecimalOne()).pow(0.1), "", "x"],
	...
	["Higher Order", "multiply the first effect of advanced components based on your component tiers", () => player.C.tiers.div(2).add(1), "", "x"],
]];

const gemUpgrades = [
	["Gem Influence", "increase faction coin find chance based on your total gems this life", () => {
	...
	["Magic Residue", "multiply the effect of Magic Gems based on your best gems this life", () => player.G.best.add(1).log10().add(1).pow(0.25), "", "x"],
];
