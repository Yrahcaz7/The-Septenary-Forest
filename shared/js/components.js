...
function loadVue(mainPage = false) {
	app = Vue.createApp({
		...
		template: template(/*html*/`<div id="app">
			<!-- canvas -->
			...
			<div v-if="false" class="fullWidth" style="display: flex; flex-direction: column; justify-content: center; background-color: #0f0f0f; z-index: 999999999999999">
				<div>
					<h1>Loading...</h1>
					<br>
					<h2>If this takes too long it means there was a serious error!</h2>
				</div>
			</div>
			<!-- game end screen -->
			<div v-if="gameEnded" class="fullWidth">
				...
				<br><br>
				<h3 v-html="modInfo.winText"></h3>
				<br><br>
				<div v-if="!player.timePlayedReset">It took you {{ formatTime(player.timePlayed) }} to beat the game.</div>
				<br>
				<button class="longUpg can" style="margin-right: 18px" @click="hardReset(true)">Play Again</button>
			...
			<div id="treeOverlay" v-if="
				!gameEnded
				&& (player.tab === 'none' || tmp.other.splitScreen || !readData(layoutInfo.showTree))
			" @scroll.passive="resizeCanvas()" :class="{
				treeOverlay: true,
			...
			}" :style="(!readData(layoutInfo.showTree) && player.tab === 'info-tab') ? {'margin-top': '50px'} : {}">
				<div id="version" @click="showTab('changelog-tab')" class="overlayThing" style="margin-right: 13px">
					{{ VERSION.withoutName }}
				</div>
				<button v-if="
					(player.navTab == 'none'
						&& (tmp[player.tab].row == 'side' || tmp[player.tab].row == 'otherside' || player[player.tab].prevTab)
					) || player[player.navTab]?.prevTab
				" class="big back overlayThing" @click="goBack(player.navTab === 'none' ? player.tab : player.navTab)">
					&#8592;
				</button>
			...
			<div v-if="
				!gameEnded
				&& (player.tab === 'none' || tmp.other.splitScreen)
			" id="treeTab" @scroll.passive="resizeCanvas()" :class="{
				fullWidth: player.tab === 'none' || player.navTab === 'none',
				col: player.tab !== 'none' && player.navTab !== 'none',
				left: player.tab !== 'none' && player.navTab !== 'none',
			}">
			...
			<div id="particle-container">
				<particle v-for="p in particles" :data="p"></particle>
			</div>
			<!-- layer tab -->
			<div v-if="
				player.navTab !== 'none'
	...
	// data = a function returning the HTML content
	addNormalComponent('display-text', {
		props: ['layer', 'data'],
		template: template(/*html*/`<span class="instant" v-html="data"></span>`),
	});
	...
	addNormalComponent('custom-resource-display', {
		props: ['layer', 'data'],
		data() { return {run} },
		template: template(/*html*/`<div class="instant" style="margin-top: -13px" v-html="'<br>' + run(data)"></div>`),
	});
	...
	addNormalComponent('display-image', {
		props: ['layer', 'data'],
		template: template(/*html*/`<img class="instant" :src="data" :alt="data">`),
	});
	...
	addNormalComponent('layer-proxy', {
		props: ['layer', 'data'],
		template: template(/*html*/`<div><column :layer="data[0]" :data="data[1]"></column></div>`),
	});
	...
	addNormalComponent('infobox', {
		...
		template: template(/*html*/`<div class="story instant" v-if="
			tmp[layer].infoboxes
			&& tmp[layer].infoboxes[data] !== undefined
			&& tmp[layer].infoboxes[data].unlocked
		" :style="[{
			'border-bottom-style': (player.infoboxes[layer][data] ? 'solid' : 'none'),
			'border-color': tmp[layer].color,
		}, tmp[layer].infoboxes[data].style]">
			<button class="story-title" :style="[
				...
				<span v-html="tmp[layer].infoboxes[data].title || tmp[layer].name"></span>
			</button>
	...
	addNormalComponent('challenges', {
		props: ['layer', 'data'],
		data() { return {tmp} },
		template: template(/*html*/`<div v-if="tmp[layer].challenges" class="upgCol">
	...
	addNormalComponent('challenge', {
		...
		" :class="[
			...
			<br><br>
			<button :class="{longUpg: true, can: canUseChallenge(layer, data), [layer]: true}" :style="{'background-color': tmp[layer].color}" @click="startChallenge(layer, data)">
			...
			<template v-else>
				<span v-html="tmp[layer].challenges[data].challengeDescription"></span>
				<br>
				Goal: <span v-if="tmp[layer].challenges[data].goalDescription" v-html="tmp[layer].challenges[data].goalDescription"></span>
				<template v-else>
					{{ format(tmp[layer].challenges[data].goal) }} {{ tmp[layer].challenges[data].currencyDisplayName || (modInfo.pointsName || "points") }}
				</template>
				<br>
				...
				<template v-if="layers[layer].challenges[data].rewardDisplay !== undefined">
					...
					<span v-html="tmp[layer].challenges[data].rewardDisplay ?
						run(layers[layer].challenges[data].rewardDisplay, layers[layer].challenges[data], tmp[layer].challenges[data].rewardEffect)
	...
	addNormalComponent('toggle', {
		...
		template: template(/*html*/`<button class="smallUpg can" :style="{'background-color': tmp[data[0]].color}" @click="toggleAuto(data)">
			{{ formatOpt(player[data[0]][data[1]]) }}
		</button>`),
	});
	...
	addNormalComponent('main-display', {
		props: ['layer', 'data'],
		data() { return {player, tmp, format, formatWhole} },
		computed: {
			extraMainDisplay() {
		...
		template: template(/*html*/`<div>
			...
			{{ tmp[layer].resource }}
			<span v-if="effectDescription" v-html="', ' + effectDescription"></span>
			<br><br>
		</div>`),
	});
	...
	addNormalComponent('respec-button', {
		...
		template: template(/*html*/`<div v-if="
			tmp[layer].buyables
			&& tmp[layer].buyables.respec
			&& (tmp[layer].buyables.showRespec === undefined || tmp[layer].buyables.showRespec)
		">
			...
			<button @click="respecBuyables(layer)" :class="{
				longUpg: true,
				can: player[layer].unlocked,
				locked: !player[layer].unlocked
			}" style="margin-right: 18px">
				{{ tmp[layer].buyables.respecText || "Respec" }}
			</button>
		</div>`),
	});
	...
	addNormalComponent('clickables', {
		props: ['layer', 'data'],
		data() { return {tmp} },
		template: template(/*html*/`<div v-if="tmp[layer].clickables" class="upgCol">
			<master-button v-if="
	...
	addNormalComponent('master-button', {
		...
		template: template(/*html*/`<button v-if="
			...
			&& tmp[layer].clickables.masterButtonPress
			&& (tmp[layer].clickables.showMasterButton === undefined || tmp[layer].clickables.showMasterButton)
		" @click="run(tmp[layer].clickables.masterButtonPress, tmp[layer].clickables)" :class="{
			longUpg: true,
			can: player[layer].unlocked,
			locked: !player[layer].unlocked,
		}">
			{{ tmp[layer].clickables.masterButtonText || "Click me!" }}
		</button>`),
	});
	...
	addNormalComponent('contained-grid', {
		props: ['layer', 'data'],
		data() { return {tmp, run, layers} },
		template: template(/*html*/`<div v-if="tmp[layer].grid" class="upgCol" :style="{
			width: 'fit-content',
			'max-width': (Array.isArray(data) ? data[0] : data),
			overflow: 'auto',
			'overscroll-behavior-x': 'none',
		}">
			<div v-for="row in (Array.isArray(data) ?
				data[1]
	...
	addNormalComponent('gridable', {
		...
		methods: {
			start() {
				if (this.interval || !layers[this.layer].grid.onHold) return;
				this.interval = setInterval((function() {
		...
		template: template(/*html*/`<button v-if="
			tmp[layer].grid
			&& player[layer].grid[data] !== undefined
			&& run(layers[layer].grid.getUnlocked, layers[layer].grid, data)
		" :class="{
			tile: true,
			can: canClick,
			locked: !canClick,
	...
	addNormalComponent('bar', {
		...
		template: template(/*html*/`<div v-if="tmp[layer].bars && tmp[layer].bars[data].unlocked" style="position: relative">
			<div :style="[
				...
				<div v-for="node in row" class="stackingContext">
					...
					<div v-if="layoutInfo.htmlBranches" class="branches">
						<template v-for="branch in tmp[node].branches">
							<div v-if="
								nodeShown(node)
								&& nodeShown(Array.isArray(branch) ? branch[0] : branch)
							" class="branch" :style="getNormalHTMLBranchStyles(branch)" ref="branch" :data-id="node" :data-data="branch"></div>
						</template>
					</div>
				</div>
			</div>
				...
				<div v-for="id in row" class="stackingContext">
					...
					<div v-if="layoutInfo.htmlBranches" class="branches">
						<template v-for="branch in tmp[layer][type + 's'][id].branches">
							<div v-if="
								tmp[layer][type + 's'][Array.isArray(branch) ?
									branch[0]
									: branch].unlocked
							" class="branch" :style="getNormalHTMLBranchStyles(branch)" ref="branch" :data-id="id" :data-data="branch"></div>
						</template>
					</div>
				</div>
			</div>
	...
	addNormalComponent('text-input', {
		props: ['layer', 'data'],
		data() { return {player, focused} },
		methods: {
			updateValue() {
				const value = document.getElementById('input-' + this.layer + '-' + this.data).value;
				player[this.layer][this.data] = toValue(value, player[this.layer][this.data]);
			},
		},
		template: template(/*html*/`<input class="instant" :id="'input-' + layer + '-' + data" :value="player[layer][data].toString()" @focus="focused = true" @blur="focused = false" @change="updateValue()">`),
	});
	...
	addNormalComponent('slider', {
		props: ['layer', 'data'],
		data() { return {player} },
		template: template(/*html*/`<div class="tooltipBox">
	...
	addNormalComponent('drop-down', {
		props: ['layer', 'data'],
		data() { return {player} },
		template: template(/*html*/`<select v-model="player[layer][data[0]]">
			<option v-for="item in data[1]" :value="item">
				{{ item }}
			</option>
		</select>`),
	});
	...
	addNormalComponent('sell-one', {
		...
		" @click="run(tmp[layer].buyables[data].sellOne, tmp[layer].buyables[data])" :class="{
			longUpg: true,
			can: player[layer].unlocked,
			locked: !player[layer].unlocked,
		}">
			{{ tmp[layer].buyables.sellOneText || "Sell One" }}
		</button>`),
	});
	...
	addNormalComponent('sell-all', {
		...
		" @click="run(tmp[layer].buyables[data].sellAll, tmp[layer].buyables[data])" :class="{
			longUpg: true,
			can: player[layer].unlocked,
			locked: !player[layer].unlocked,
		}">
			{{ tmp[layer].buyables.sellAllText || "Sell All" }}
		</button>`),
	});
	...
	addNormalComponent('tab-buttons', {
		...
		data() { return {subtabShouldNotify, subtabResetNotify, tmp, defaultGlow} },
		methods: {
			changeTab(tab) {
				player.subtabs[this.layer][this.name] = tab;
				updateTabFormats();
				needCanvasUpdate = true;
			},
		},
		template: template(/*html*/`<div class="upgRow">
	...
	addNormalComponent('tree-node', {
		...
		template: template(/*html*/`<button v-if="nodeShown(layer)" :id="layer + (isAlias ? '-alias' : '')" @click="onClick" :class="{
	...
	addNormalComponent('options-tab', {
		...
		template: template(/*html*/`<table><tbody>
	...
	addNormalComponent('particle', {
		...
		template: template(/*html*/`<div class="particle instant" :style="[
	...
	addNormalComponent('bg', {
		props: ['layer'],
		data() { return {tmp, player} },
		template: template(/*html*/`<div class="bg" :style="[
			tmp[layer].style || {},
			(tmp[layer].tabFormat && !Array.isArray(tmp[layer].tabFormat) ?
				tmp[layer].tabFormat[player.subtabs[layer].mainTabs].style
				: {}),
