...
addLayer("a", {
	...
	startData() { return {
		...
		population: newDecimalOne(),
		populationMax: newDecimalOne(),
		populationTime: 0,
		autoCRA: false,
		autoFER: false,
		autoANA: false,
	...
	effect() {
		...
		// multiply
		let mult = [newDecimalOne(), newDecimalOne(), newDecimalOne()];
	...
	doReset(resettingLayer) {
		...
		const keep = ["autoCRA", "autoFER", "autoANA", "autoSOV"];
	...
	automate() {
		if (player.ec.unlocked) {
			if (player.a.autoCRA) buyBuyable("a", 11);
			if (player.a.autoFER) buyBuyable("a", 12);
			if (player.a.autoANA) buyBuyable("a", 13);
			if (player.a.autoSOV) buyBuyable("a", 14);
		};
	},
	componentStyles: {
	...
	buyables: {
		...
		respec() {
			setBuyableAmount("a", 11, newDecimalZero());
			setBuyableAmount("a", 12, newDecimalZero());
			setBuyableAmount("a", 13, newDecimalZero());
			setBuyableAmount("a", 14, newDecimalZero());
			player.a.spent = newDecimalZero();
	...
	milestones: {
		0: {
			...
			effectDescription(eff) {return "multiply the base effect of CRA based on acclimation points<br>Effect: " + format(eff) + "x<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
		...
		2: {
			...
			effectDescription(eff) {return "multiply the base effect of SOV based on acclimation points<br>Effect: " + format(eff) + "x<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.d.unlocked},
		},
		3: {
		...
		15: {
			...
			effectDescription(eff) {return "multiply the base effect of FER based on acclimation points<br>Effect: " + format(eff) + "x<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.d.unlocked},
		},
		16: {
			...
			effectDescription(eff) {return "increase the base of the 7th retrogression's last effect based on acclimation points<br>Effect: +" + format(eff) + "<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.d.unlocked},
		},
		17: {
			...
			effectDescription(eff) {return "multiply the last population effect based on acclimation points<br>Effect: " + format(eff) + "x<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.d.unlocked},
		},
		18: {
			...
			effectDescription(eff) {return "every " + (hasMilestone("a", 32) ? 2 : 3) + " base levels of CRA give an extra level to FER, ANA, and SOV<br>Effect: +" + formatWhole(eff) + "<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.d.unlocked},
		},
		19: {
		...
		20: {
			...
			effectDescription(eff) {return "every " + (hasMilestone("a", 32) ? 2 : 3) + " base levels of ANA give an extra level to CRA, FER, and SOV<br>Effect: +" + formatWhole(eff) + "<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.d.unlocked},
		},
		21: {
		...
		22: {
			...
			effectDescription(eff) {return "every " + (hasMilestone("a", 32) ? 2 : 3) + " base levels of SOV give an extra level to CRA, FER, and ANA<br>Effect: +" + formatWhole(eff) + "<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.d.unlocked},
		},
		23: {
		...
		24: {
			...
			effectDescription(eff) {return "every " + (hasMilestone("a", 32) ? 2 : 3) + " base levels of FER give an extra level to CRA, ANA, and SOV<br>Effect: +" + formatWhole(eff) + "<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.d.unlocked},
		},
		25: {
			...
			effectDescription(eff) {return "multiply the last two population effects based on acclimation points<br>Effect: " + format(eff) + "x<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.d.unlocked},
		},
		26: {
		...
		31: {
			...
			effectDescription(eff) {return "multiply the exponent of the first population effect based on acclimation points<br>Effect: " + format(eff) + "x<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.d.unlocked},
		},
		32: {
		...
		43: {
			...
			effectDescription(eff) {return "every 4 base levels of CRA, FER, ANA, and SOV give an extra level to CRA, FER, ANA, and SOV<br>Effect: +" + formatWhole(eff) + "<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.d.unlocked},
		},
		44: {
		...
		50: {
			...
			popupTitle: "Enhancement Acquired!",
			effect() {return player.a.points.add(1).pow(0.25)},
			effectDescription(eff) {return "divide focus requirement based on acclimation points<br>Effect: /" + format(eff) + "<br>Req: " + formatWhole(this.requirement) + " acclimation points and 35 completions of the 10th retrogression"},
		...
		59: {
			...
			effectDescription(eff) {return "multiply the base effect of CRA based on acclimation points<br>and reduce population max's influence on gain<br>Effects: " + format(eff) + "x and -0.03<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.ec.unlocked},
		},
		60: {
		...
		66: {
			...
			effectDescription(eff) {return "multiply the base effect of CRA based on acclimation points<br>and reduce population max's influence on gain<br>Effects: " + format(eff) + "x and -0.14<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.ec.unlocked},
		},
		67: {
			...
			effectDescription(eff) {return "every " + (hasMilestone("a", 74) ? 2 : 15) + " base levels of FER give an extra level to CRA, FER, ANA, and SOV<br>Effect: +" + formatWhole(eff) + "<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.ec.unlocked},
		},
		68: {
		...
		69: {
			...
			effectDescription(eff) {return "divide domination requirement based on acclimation points<br>Effect: /" + format(eff) + "<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1) || player.ec.unlocked},
		},
		70: {
			...
			effectDescription(eff) {return "increase the base effect of ANA based on acclimation points<br>Effect: +" + format(eff) + "<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
		...
		73: {
			...
			effectDescription(eff) {return "multiply the base effect of CRA based on acclimation points<br>and reduce population max's influence on gain<br>Effects: " + format(eff) + "x and -0.17<br>Req: " + formatWhole(this.requirement) + " acclimation points"},
			done() {return player.a.points.gte(this.requirement)},
			unlocked() {return hasMilestone("a", this.id - 1)},
		},
		74: {
