...
- `hotkeys`: **optional**. An array containing information on any hotkeys associated with this layer:
    ...
    hotkeys: [
        {
            key: "p", // What the hotkey button is. Use uppercase if it's combined with shift, or "ctrl+x" for holding down the control key or the meta key.
...
- `style`: **optional**. A ["CSS object"](other.md#css) which styles this layer's entire tab.
...
- `nodeStyle`: **optional**. A ["CSS object"](other.md#css) which styles this layer's node on the tree.
